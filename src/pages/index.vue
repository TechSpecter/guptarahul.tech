<template>
  <main id="terminal" class="p-4 text-white bg-base-200 h-full overflow-scroll" @click="clickOnWrapper">
    <div id="greentings" class="mb-3">
      <p>Hello, I'm Rahul <span class="text-purple-500">&lt;3</span> Welcome!</p>
    ...
    <div id="output">
      <template v-for="(output, index) in outputList" :key="index">
        <span class="text-primary">guest@guptarahul.tech <span>{{output.date}}</span><span class="text-secondary"> ~/ <span class="text-white">$</span></span><span class="text-white">&nbsp;{{output.commandInput}}</span></span>
        <div v-html="output.html"/>
      </template>
    </div>
    <div id="command">
      <form @submit.prevent="submit">
        <span class="text-primary">guest@guptarahul.tech <span>{{currentDate}}</span><span class="text-secondary"> ~/ <span class="text-white">$</span></span></span>
        <input
          v-model="inputModel"
          autocomplete="off"
          id="prompt-input"
